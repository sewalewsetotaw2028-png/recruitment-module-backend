...
async function main() {
  try {
    ...
    const appMap: Record<string, any> = applications.reduce(
      (map: Record<string, any>, app: any) => {
        map[app.id] = app;
        return map;
      },
      {},
    ...
    const standardTemplate = await prisma.interviewEvaluationTemplate.upsert({
      where: {
        company_id_name: { company_id: company.id, name: 'Standard Interview' },
      },
    ...
    await prisma.evaluationCriteria.deleteMany({
      where: { template_id: hrEvalTemplate.id },
    });
    await prisma.evaluationCriteria.createMany({
      data: [
        {
    ...
    await prisma.evaluationCriteria.deleteMany({
      where: { template_id: techEvalTemplate.id },
    });
    await prisma.evaluationCriteria.createMany({
      data: [
        {
    ...
    await prisma.evaluationCriteria.deleteMany({
      where: { template_id: mgrEvalTemplate.id },
    });
    await prisma.evaluationCriteria.createMany({
      data: [
        {
    ...
    // ── 3. Question Banks ───────────────────────────────────────────────────────
    // NOTE: relies on the new @@unique([company_id, title]) on InterviewQuestionBank —
    // make sure you've run the schema migration that adds it before this seed.
    const techBank = await prisma.interviewQuestionBank.upsert({
      where: { id: 'backend-core-questions' } as any,
      update: {},
      create: {
        id: 'backend-core-questions',
        company_id: company.id,
    ...
    const hrBank = await prisma.interviewQuestionBank.upsert({
      where: { id: 'hr-behavioral-questions' } as any,
      update: {},
      create: {
    ...
    const appIntSched1 = applications.find(
      (a) => a.status === 'INTERVIEW_SCHEDULED',
    );
    const appIntSched2 = applications.find(
      (a, i) =>
        a.status === 'INTERVIEW_SCHEDULED' &&
        i !== applications.indexOf(appIntSched1!),
    );
    const appIntComp1 = applications.find(
      (a) => a.status === 'INTERVIEW_COMPLETED',
    );
    const appIntComp2 = applications.find(
      (a, i) =>
        a.status === 'INTERVIEW_COMPLETED' &&
        i !== applications.indexOf(appIntComp1!),
    );
    const appEval1 = applications.find((a) => a.status === 'UNDER_EVALUATION');
    const appEval2 = applications.find(
      (a, i) =>
        a.status === 'UNDER_EVALUATION' &&
        i !== applications.indexOf(appEval1!),
    );

    // Interview.application_id is a required field, so `.find()` returning undefined
    // must fail loudly here rather than silently passing `undefined` through (which
    // both breaks the TS types and would otherwise insert an invalid interview).
    function requireApp(
      ...
      if (!app) {
        throw new Error(
          `Seed data missing: no application with the expected status found for "${label}". ` +
            `Make sure 03-candidate-data.ts ran first and seeded that status.`,
        );
      }
      return app.id;
    }
    ...
    const intSched1 = await prisma.interview.upsert({
      ...
      update: {},
      create: {
        application_id: requireApp(appIntSched1, 'INTERVIEW_SCHEDULED #1'),
    ...
    const intResch1 = await prisma.interview.upsert({
      ...
      update: {},
      create: {
        application_id: requireApp(appIntSched2, 'INTERVIEW_SCHEDULED #2'),
        interview_number: nextIntNum(),
        round: 1,
        interview_category_id: catTech.id,
    ...
    const intComp1 = await prisma.interview.upsert({
      ...
      update: {},
      create: {
        application_id: requireApp(appIntComp1, 'INTERVIEW_COMPLETED #1'),
        interview_number: nextIntNum(),
        round: 1,
        interview_category_id: catTech.id,
        start_time: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000),
    ...
    // NOTE: relies on the new @@unique([interview_id, evaluator_id]) on
    // InterviewEvaluation — make sure the schema migration adding it has run.
    await prisma.interviewEvaluation.upsert({
      where: { id: 'eval-comp1-interviewer2' } as any,
      update: {},
      create: {
        id: 'eval-comp1-interviewer2',
        interview_id: intComp1.id,
    ...
    const intCancel1 = await prisma.interview.upsert({
      ...
      update: {},
      create: {
        application_id: requireApp(appIntComp2, 'INTERVIEW_COMPLETED #2'),
        interview_number: nextIntNum(),
        round: 1,
        interview_category_id: catHR.id,
        start_time: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000),
    ...
    await prisma.interviewPanel.upsert({
      where: {
        interview_id_panel_member_id: {
          ...
          panel_member_id: userMap['interviewer3@erms.com'].id,
        },
      },
      update: {},
      create: {
    ...
    const intEvalPending = await prisma.interview.upsert({
      ...
      update: {},
      create: {
        application_id: requireApp(appEval1, 'UNDER_EVALUATION #1'),
        interview_number: nextIntNum(),
        round: 1,
        interview_category_id: catMgr.id,
    ...
    await prisma.interviewPanel.upsert({
      where: {
        interview_id_panel_member_id: {
          ...
          panel_member_id: userMap['hm1@erms.com'].id,
        },
      },
      update: {},
      create: {
    ...
    const intFinalized = await prisma.interview.upsert({
      ...
      update: {},
      create: {
        application_id: requireApp(appEval2, 'UNDER_EVALUATION #2'),
        interview_number: nextIntNum(),
        round: 2,
    ...
    await prisma.interviewPanel.upsert({
      where: {
        interview_id_panel_member_id: {
          ...
          panel_member_id: userMap['hm2@erms.com'].id,
        },
      },
      update: {},
      create: {
    ...
    await prisma.interviewEvaluation.upsert({
      where: { id: 'eval-finalized-hm2' } as any,
      update: {},
      create: {
        id: 'eval-finalized-hm2',
        interview_id: intFinalized.id,
