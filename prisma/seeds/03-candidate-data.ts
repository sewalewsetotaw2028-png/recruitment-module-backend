...
async function main() {
  try {
    ...
    const recruitmentSourceMap: Record<string, any> = sources.reduce(
      (map: Record<string, any>, source: any) => {
    ...
    for (const cand of candidateData) {
      ...
      // Experience: replace upsert with findFirst + create because Experience has no composite unique defined
      const existingExp = await prisma.experience.findFirst({
        where: { candidate_id: dbCand.id, company_name: 'Tech Ethiopia PLC' },
      });
      if (!existingExp) {
      ...
      const existingCert = await prisma.candidateCertification.findFirst({
        where: {
          candidate_id: dbCand.id,
      ...
      // Candidate Document (unique on candidate_id)
      const existingDoc = await prisma.candidateDocument.findUnique({
        where: { candidate_id: dbCand.id },
      });
      if (!existingDoc) {
      ...
      // Phone
      const existingCandPhone = await prisma.phone.findFirst({
        where: { candidate_id: dbCand.id, phone_type: 'PRIVATE' },
      });
      if (!existingCandPhone) {
      ...
      // Address
      const existingCandAddress = await prisma.address.findFirst({
        where: { candidate_id: dbCand.id },
      });
      if (!existingCandAddress) {
    ...
    for (const cpUser of candidatePortalDefs) {
      ...
      for (const exp of cpUser.experiences) {
        const existingExp = await prisma.experience.findFirst({
          where: { candidate_id: candidate.id, company_name: exp.company },
        });
        if (!existingExp) {
          await prisma.experience.create({
            data: {
      ...
      const existingPhone = await prisma.phone.findFirst({
        where: { candidate_id: candidate.id, phone_type: 'PRIVATE' },
      });
      if (!existingPhone) {
        await prisma.phone.create({
          data: {
      ...
      const app = await prisma.application.upsert({
        ...
        create: {
          ...
          recruitment_source_id: sourceChannel
            ? (recruitmentSourceMap[sourceChannel]?.id ?? null)
            : null,
